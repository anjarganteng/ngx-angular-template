...
@Component({
  template: `
    <div style="gap:20px;" class="d-flex flex-row"  >
      ...
      <button class="btn btn-danger" (click)="open({
                                                      title:'Reject',
...
export class ActionsComponent implements ViewCell {
  ...
  onApprove() {
    ...
    this.api.post(this.api.urlBase+'/trxYearIeMaterial/v1/approve', this.rowData.yearIE)
    .subscribe((res: any) => {
      this.actionService.action({action:'approve', data: this.rowData});
      this.toastrService.success(`Item ${this.rowData.yearIE} Approve successfully`, 'Success')
    }, (err:any) => {
      this.toastrService.danger(`Item ${this.rowData.yearIE} Approve failed`, 'Error');
    })
  }

  onReject() {
    this.reject.emit(this.rowData);
    this.api.post(this.api.urlBase+'/trxYearIeMaterial/v1/reject', this.rowData.yearIE)
    .subscribe((res: any) => {
      this.toastrService.success(`Item ${this.rowData.yearIE} reject successfully`, 'Success')
    }, (err:any) => {
      this.toastrService.danger(`Item ${this.rowData.yearIE} failed to reject`, 'Error');
    })
  }
